...
def upload():
    try:
        ...
        if filepath:#если не пустая то выполнить ниже
            files={"file":open(filepath,'rb')}## Создаем словарь с данными для отправки, где ключ "file" и значение - файл в бинарном режимеоткр файл для чтения по байтно для отправкив сеть 'rb' - Открывает файл в бинарном режиме только для чтения. Указатель файла помещается в начале файла. Это режим "по умолчанию".
